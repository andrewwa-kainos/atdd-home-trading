...
public class ApiDriver implements Driver {
    ...
    private final Map<String, HttpResponse<String>> ordersPlaced;
    private final Map<String, HttpResponse<String>> ordersViewed;
    private final Map<String, HttpResponse<String>> ordersCancelled;
    ...
    private static void registerOrderResponse(
            Map<String, HttpResponse<String>> map,
            String orderNumber,
    ...
    public void confirmOrderPlaced(String order, String prefix) {
        ...
        assertNotNull(response.orderNumber(), "Order number should be not be null");
        assertFalse(response.orderNumber().isEmpty(), "Order number should be not be empty");
        assertTrue(response.orderNumber().startsWith(prefix), "Order number should start with prefix: " + prefix);
    }
    ...
    public void confirmOrderDetails(String order, String productId, String quantity, String status) {
        ...
        assertEquals(Long.parseLong(productId), response.productId());
        assertEquals(Long.parseLong(quantity), response.quantity());
        ...
        assertEquals(status, response.status(), "Order status should be: " + status);
    }
    ...
    public void confirmOrderStatusIsCancelled(String order) {
        ...
        var httpResponse = ordersViewed.get(order);
        var response = apiClient.getOrderController().confirmOrderViewedSuccessfully(httpResponse);
        assertEquals("CANCELLED", response.status(), "Order status should be CANCELLED");
    }
